	...
	int theX, int theY, int theRenderOrder, int theRow)
{
	...
	if (*mType == OldProjectileType::PROJECTILE_CABBAGE || *mType == OldProjectileType::PROJECTILE_BUTTER)
	{
		mRotation = DEG_TO_RAD(-50.4f);
		mRotationSpeed = RandRangeFloat(-0.08f, -0.02f);
	}
	else if (*mType == OldProjectileType::PROJECTILE_MELON ||
			 *mType == OldProjectileType::PROJECTILE_WINTERMELON)
	{
		mRotation = DEG_TO_RAD(-72.0f);
		mRotationSpeed = RandRangeFloat(-0.08f, -0.02f);
	}
	else if (*mType == OldProjectileType::PROJECTILE_KERNEL)
	{
		mRotation = 0.0f;
		mRotationSpeed = RandRangeFloat(-0.2f, -0.08f);
	}
	else if (*mType == OldProjectileType::PROJECTILE_SNOWPEA)
	{
	...
	else if (*mType == OldProjectileType::PROJECTILE_FIREBALL)
	{
		TOD_ASSERT();
	}
	else if (*mType == OldProjectileType::PROJECTILE_COBBIG)
	{
		mWidth = IMAGE_REANIM_COBCANNON_COB->GetWidth();
		mHeight = IMAGE_REANIM_COBCANNON_COB->GetHeight();
		mRotation = PI / 2;
	}
	else if (*mType == OldProjectileType::PROJECTILE_PUFF)
	{
		TodParticleSystem *aParticle =
	...
	else if (*mType == OldProjectileType::PROJECTILE_BASKETBALL)
	{
		mRotation = RandRangeFloat(0.0f, 2 * PI);
		mRotationSpeed = RandRangeFloat(0.05f, 0.1f);
	}
	else if (*mType == OldProjectileType::PROJECTILE_STAR)
	{
		mShadowY += 15.0f;
		mRotationSpeed = RandRangeFloat(0.05f, 0.1f);
		if (Rand(2) == 0)
...
Plant *Projectile::FindCollisionTargetPlant()
{
	...
	while (mBoard->IteratePlants(aPlant))
	{
		...
		if (*mType == OldProjectileType::PROJECTILE_ZOMBIE_PEA)
		{
...
bool Projectile::PeaAboutToHitTorchwood()
{
	...
	if (*mType != OldProjectileType::PROJECTILE_PEA && *mType != OldProjectileType::PROJECTILE_SNOWPEA)
		return false;
...
Zombie *Projectile::FindCollisionTarget()
{
	...
	while (mBoard->IterateZombies(aZombie))
	{
		...
		if ((aZombie->mZombieType == ZombieType::ZOMBIE_BOSS || aZombie->mRow == mRow) && isEffected)
		{
			...
			if (*mType == OldProjectileType::PROJECTILE_STAR && mProjectileAge < 25 && mVelX >= 0.0f &&
...
void Projectile::CheckForCollision()
{
	...
	if ((*mType == OldProjectileType::PROJECTILE_PEA || *mType == OldProjectileType::PROJECTILE_STAR) &&
	...
	if (aZombie)
	{
		if ((aZombie->mOnHighGround && CantHitHighGround()) ||
			(*mType == OldProjectileType::PROJECTILE_ZOMBIE_PEA && !aZombie->mMindControlled))
		{
			return;
		}
...
bool Projectile::CantHitHighGround()
{
	...
	return (*mType == OldProjectileType::PROJECTILE_PEA ||
			*mType == OldProjectileType::PROJECTILE_SNOWPEA ||
			*mType == OldProjectileType::PROJECTILE_STAR || *mType == OldProjectileType::PROJECTILE_PUFF ||
...
void Projectile::CheckForHighGround()
{
	...
	if (*mType == OldProjectileType::PROJECTILE_PEA || *mType == OldProjectileType::PROJECTILE_SNOWPEA ||
		*mType == OldProjectileType::PROJECTILE_FIREBALL || *mType == OldProjectileType::PROJECTILE_SPIKE ||
		*mType == OldProjectileType::PROJECTILE_COBBIG)
	{
	...
	if (*mType == OldProjectileType::PROJECTILE_PUFF && aShadowDelta < 0.0f)
	{
		DoImpact(nullptr);
		return;
	}

	if (*mType == OldProjectileType::PROJECTILE_STAR && aShadowDelta < 23.0f)
	{
		DoImpact(nullptr);
		return;
	}
...
{
	if (*mType == PROJECTILE_FIREBALL && theZombie && theZombie->IsFireResistant())
		return false;

	return *mType == OldProjectileType::PROJECTILE_MELON ||
		   *mType == OldProjectileType::PROJECTILE_WINTERMELON ||
		   *mType == OldProjectileType::PROJECTILE_FIREBALL;
}
...
unsigned int Projectile::GetDamageFlags(Zombie *theZombie)
{
	...
	if (*mType == OldProjectileType::PROJECTILE_SNOWPEA ||
		*mType == OldProjectileType::PROJECTILE_WINTERMELON)
	{
...
{
	Rect aProjectileRect = GetProjectileRect();
	if (*mType == OldProjectileType::PROJECTILE_FIREBALL)
	{
	...
	if (theZombie->IsFireResistant() && *mType == OldProjectileType::PROJECTILE_FIREBALL)
	{
		return false;
	}
...
{
	if (*mType == OldProjectileType::PROJECTILE_COBBIG && mPosZ < -700.0f)
	{
		mVelZ = 8.0f;
		mRow = mCobTargetRow;
		mPosX = mCobTargetX;
	...
	if (isRising && (*mType == OldProjectileType::PROJECTILE_BASKETBALL ||
					 *mType == OldProjectileType::PROJECTILE_COBBIG))
	{
		return;
	}
	if (mProjectileAge > 20)
	{
		...
		if (*mType == OldProjectileType::PROJECTILE_BUTTER)
		{
		...
		else if (*mType == OldProjectileType::PROJECTILE_CABBAGE ||
				 *mType == OldProjectileType::PROJECTILE_KERNEL)
		{
	...
	Plant *aPlant = nullptr;
	Zombie *aZombie = nullptr;
	if (*mType == OldProjectileType::PROJECTILE_BASKETBALL ||
		*mType == OldProjectileType::PROJECTILE_ZOMBIE_PEA)
	{
	...
	if (*mType == OldProjectileType::PROJECTILE_COBBIG)
	{
		aGroundZ = -40.0f;
	}
...
void Projectile::PlayImpactSound(Zombie *theZombie)
{
	...
	if (*mType == OldProjectileType::PROJECTILE_KERNEL)
	{
		mApp->PlayFoley(FoleyType::FOLEY_KERNEL_SPLAT);
		aPlayHelmSound = false;
		aPlaySplatSound = false;
	}
	else if (*mType == OldProjectileType::PROJECTILE_BUTTER)
	{
		mApp->PlayFoley(FoleyType::FOLEY_BUTTER);
		aPlaySplatSound = false;
	}
	else if (*mType == OldProjectileType::PROJECTILE_FIREBALL && IsSplashDamage(theZombie))
	{
...
void Projectile::DoImpact(Zombie *theZombie)
{
	...
	if (IsSplashDamage(theZombie))
	{
		if (*mType == OldProjectileType::PROJECTILE_FIREBALL && theZombie)
		{
	...
	if (*mType == OldProjectileType::PROJECTILE_MELON)
	{
		mApp->AddTodParticle(
			aLastPosX + 30.0f, aLastPosY + 30.0f, mRenderOrder + 1, ParticleEffect::PARTICLE_MELONSPLASH);
	}
	else if (*mType == OldProjectileType::PROJECTILE_WINTERMELON)
	{
		mApp->AddTodParticle(
	...
	else if (*mType == OldProjectileType::PROJECTILE_PEA)
	{
		aSplatPosX -= 15.0f;
...
void Projectile::Update()
{
	...
	int aTime = 20;
	if (*mType != OldProjectileType::PROJECTILE_PUFF &&
		*mType != OldProjectileType::PROJECTILE_FIREBALL &&
		*mType != OldProjectileType::PROJECTILE_STAR &&
		*mType != OldProjectileType::PROJECTILE_BASKETBALL)
	{
		aTime = 0;
	}
...
void Projectile::Draw(Graphics *g)
{
	...
	else if (*mType == OldProjectileType::PROJECTILE_SPIKE)
	{
...
void Projectile::Die()
{
	...
	if (*mType == OldProjectileType::PROJECTILE_PUFF || *mType == OldProjectileType::PROJECTILE_SNOWPEA)
	{
...
{
	ProjectileDefinition &aProjectileDef = gProjectileDefinition[(int)mType];
	TOD_ASSERT(aProjectileDef.mType == *mType);
