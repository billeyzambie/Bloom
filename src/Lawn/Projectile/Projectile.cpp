...
bool Projectile::PeaAboutToHitTorchwood()
{
	...
	if (mType != ProjectileTypes::PEA && mType != ProjectileTypes::SNOWPEA)
		return false;
...
Zombie *Projectile::FindCollisionTarget()
{
	...
	while (mBoard->IterateZombies(aZombie))
	{
		...
		if ((aZombie->mZombieType == ZombieType::ZOMBIE_BOSS || aZombie->mRow == mRow) && isEffected)
		{
			...
			if (mType == ProjectileTypes::STAR && mProjectileAge < 25 && mVelX >= 0.0f &&
...
void Projectile::CheckForCollision()
{
	...
	if (aZombie)
	{
		if ((aZombie->mOnHighGround && CantHitHighGround()) ||
			(mType == ProjectileTypes::ZOMBIE_PEA && !aZombie->mMindControlled))
		{
			return;
		}
...
bool Projectile::CantHitHighGround()
{
	...
	return (mType == ProjectileTypes::PEA ||
			mType == ProjectileTypes::SNOWPEA ||
			mType == ProjectileTypes::STAR || mType == ProjectileTypes::PUFF ||
...
void Projectile::CheckForHighGround()
{
	...
	if (mType == ProjectileTypes::PEA || mType == ProjectileTypes::SNOWPEA ||
		mType == ProjectileTypes::FIREBALL || mType == ProjectileTypes::SPIKE ||
		mType == ProjectileTypes::COBBIG)
	{
	...
	if (mType == ProjectileTypes::PUFF && aShadowDelta < 0.0f)
	{
		DoImpact(nullptr);
		return;
	}

	if (mType == ProjectileTypes::STAR && aShadowDelta < 23.0f)
	{
		DoImpact(nullptr);
		return;
	}
...
void Projectile::UpdateLobMotion()
{
	...
	if (isRising && (mType == ProjectileTypes::BASKETBALL ||
					 mType == ProjectileTypes::COBBIG))
	{
		return;
	}
...
void Projectile::PlayImpactSound(Zombie *theZombie)
{
	...
	if (mType == ProjectileTypes::KERNEL)
	{
...
void Projectile::Draw(Graphics *g)
{
	...
	if (aImage)
	{
		...
		Rect aSrcRect(aCelWidth * mFrame, 0, aCelWidth, aCelHeight);
...
void Projectile::Die()
{
	...
	if (mType == ProjectileTypes::PUFF || mType == ProjectileTypes::SNOWPEA)
	{
