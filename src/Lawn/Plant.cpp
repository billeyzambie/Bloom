...
Plant::Plant() : GameObject(GameObjectType::OBJECT_TYPE_PLANT)
{
}
	...
	while (mBoard->IterateZombies(aZombie))
	{
		...
		if (aZombie->mOnHighGround == IsOnHighGround() && aZombie->EffectedByDamage(aDamageRangeFlags))
		{
			...
			if (GetRectOverlap(aAttackRect, aZombieRect) > 0)
			{
				...
				Damage aDamageParams = Damage::DirectlyFrom(this, aDamage, theDamageFlags);
...
void Plant::UpdateChomper()
{
	...
	else if (mState == PlantState::STATE_CHOMPER_BITING)
	{
		if (mStateCountdown == 0)
		{
			...
			{
				mApp->PlayFoley(FoleyType::FOLEY_SPLAT);
				Damage aDamage = Damage::DirectlyFrom(this, 40, 0U);
				aZombie->TakeDamage(aDamage);
...
void Plant::UpdateBowling()
{
	...
	if (aZombie)
	{
		...
		{
			Damage aDamage = Damage::DirectlyFrom(this, 400, 0U);
		...
		else if (aZombie->mHelmType != HelmType::HELMTYPE_NONE)
		{
			...
			Damage aDamage = Damage::DirectlyFrom(this, 900, 0U);
...
void Plant::Fire(Zombie *theTargetZombie, int theRow, PlantWeapon thePlantWeapon)
{
	...
	const ProjectileType *aProjectileType = nullptr;
	switch (mSeedType)
	{
	case SeedType::SEED_PEASHOOTER:
	...
	case SeedType::SEED_GATLINGPEA:
	case SeedType::SEED_LEFTPEATER:
		aProjectileType = ProjectileTypes::PEA;
		break;
	case SeedType::SEED_SNOWPEA:
		aProjectileType = ProjectileTypes::SNOWPEA;
		break;
	case SeedType::SEED_PUFFSHROOM:
	case SeedType::SEED_SCAREDYSHROOM:
	case SeedType::SEED_SEASHROOM:
		aProjectileType = ProjectileTypes::PUFF;
		break;
	case SeedType::SEED_CACTUS:
	case SeedType::SEED_CATTAIL:
		aProjectileType = ProjectileTypes::SPIKE;
		break;
	case SeedType::SEED_CABBAGEPULT:
		aProjectileType = ProjectileTypes::CABBAGE;
		break;
	case SeedType::SEED_KERNELPULT:
		aProjectileType = ProjectileTypes::KERNEL;
		break;
	case SeedType::SEED_MELONPULT:
		aProjectileType = ProjectileTypes::MELON;
		break;
	case SeedType::SEED_WINTERMELON:
		aProjectileType = ProjectileTypes::WINTERMELON;
		break;
	case SeedType::SEED_COBCANNON:
		aProjectileType = ProjectileTypes::COBBIG;
		break;
	default:
		TOD_ASSERT();
		break;
	...
	{
		aProjectileType = ProjectileTypes::BUTTER;
	}
